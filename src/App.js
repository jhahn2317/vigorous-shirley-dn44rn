...
function AppContent() {
  ...
  const [messageFormData, setMessageFormData] = useState({ author: '현아', text: '' });
  ...
  const formatCompactMoney = (val) => {
    ...
    } else result = new Intl.NumberFormat('ko-KR').format(absVal);
    return (val < 0 ? '-' : '+') + result; 
  };
  ...
  const handleEndDelivery = async () => {
    const end = new Date();
    setDeliveryFormData({ ...deliveryFormData, date: getKSTDateStr(), startTime: formatTimeStr(new Date(trackingStartTime)), endTime: formatTimeStr(end), amount: '', count: '' });
  ...
  const saveBatchDuties = async () => {
    ...
    if (isFirebaseEnabled) {
      ...
      logEvent('근무표 업데이트', `${dutyBatchMonth}월 근무표를 업데이트했습니다.`);
    }
    showToast("근무표가 저장되었습니다."); setIsDutyBatchModalOpen(false);
  };
                ...
                {logs.sort((a,b) => (b.timestamp?.seconds || 0) - (a.timestamp?.seconds || 0)).slice(0, 30).map((log) => (
                   <div key={log.id} className="text-[10px] border-b border-gray-200/50 pb-1.5 last:border-0 last:pb-0">
                      <div className="flex justify-between items-center mb-0.5">
                        ...
                        <span className="text-gray-400 font-bold">{log.timestamp ? new Date(log.timestamp.seconds * 1000).toLocaleTimeString('ko-KR', {hour12: false}) : '방금'}</span>
                      </div>
          ...
          <div className="space-y-5 animate-in fade-in duration-500">
            <div className="bg-pink-50/80 rounded-3xl p-5 border border-pink-200/60 shadow-sm relative">
               ...
               <div className="flex gap-2 relative pt-4">
                  <div className="absolute top-0 left-1 flex gap-1.5">
                    <button onClick={() => setMessageFormData({...messageFormData, author: '현아'})} className={`text-[9px] font-black px-2 py-0.5 rounded-t-lg ${messageFormData.author==='현아' ? 'bg-white text-pink-500 shadow-sm' : 'text-gray-400'}`}>현아</button>
                    <button onClick={() => setMessageFormData({...messageFormData, author: '정훈'})} className={`text-[9px] font-black px-2 py-0.5 rounded-t-lg ${messageFormData.author==='정훈' ? 'bg-white text-blue-500 shadow-sm' : 'text-gray-400'}`}>정훈</button>
                  </div>
                  <input value={messageFormData.text} onChange={e => setMessageFormData({...messageFormData, text: e.target.value})} placeholder="가벼운 한줄 메시지 🍗" className="flex-1 bg-white rounded-2xl px-4 py-3 text-sm font-bold outline-none border border-pink-200 shadow-sm" />
                  <button onClick={handleSendMessage} disabled={!messageFormData.text.trim()} className="bg-pink-500 text-white px-4 rounded-2xl font-black shadow-md active:scale-95 disabled:opacity-50">전송</button>
               </div>
            </div>
            ...
            <div className="pt-4"> 
              <div className="flex justify-between items-center mb-3 px-2">
                ...
                }} className="text-[10px] bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-full font-bold border border-emerald-200">등록/수정</button>
              </div>
              <div className="relative">
                <div ref={dutyTimelineRef} className="flex overflow-x-auto no-scrollbar gap-2 px-1 pb-2 pt-10 scroll-smooth"> 
                  {extendedDutyDays.map((d) => {
                    const duty = events.find(e => e.date === d && e.type === '듀티')?.title || 'OFF';
                    const isToday = d === todayStr;
                    let color = 'bg-white text-gray-400 border-gray-200';
                    if(duty === 'DAY') color = 'bg-blue-50 text-blue-600 border-blue-200';
                    else if(duty === 'EVE') color = 'bg-orange-50 text-orange-600 border-orange-200';
                    else if(duty !== 'OFF') color = 'bg-pink-50 text-pink-600 border-pink-200';
                    return (
                      <div key={d} id={isToday ? 'duty-today' : undefined} className={`flex-none w-[60px] p-2.5 rounded-[1.2rem] border shadow-sm flex flex-col items-center justify-center transition-all relative ${isToday ? 'ring-2 ring-emerald-400 bg-emerald-50 text-emerald-700 border-emerald-200' : color}`}>
        ...
        {activeTab === 'ledger' && (
          <div className="space-y-4 animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 grid grid-cols-3 gap-2">
              <div className="text-center"><div className="text-[10px] font-bold text-blue-500 mb-1">수입</div><div className="text-sm font-black">{formatMoney(ledgerSummary.income)}</div></div>
              <div className="text-center border-x border-gray-100"><div className="text-[10px] font-bold text-red-500 mb-1">지출</div><div className="text-sm font-black">{formatMoney(ledgerSummary.expense)}</div></div>
              <div className="text-center"><div className="text-[10px] font-bold text-indigo-500 mb-1">남은돈</div><div className="text-sm font-black text-indigo-600">{formatMoney(ledgerSummary.net)}</div></div>
            </div>
            ...
            {ledgerSubTab === 'daily' && ledgerDates.map(date => (
          ...
          <div className="space-y-6 pb-28 pt-4 animate-in slide-in-from-right duration-500">
             ...
             <section>
                <div className="flex justify-between items-center mb-4 px-2">
                  <h3 className="text-lg font-black text-gray-900">주식 자산</h3>
                  <div className="flex items-center bg-gray-100 px-3 py-1.5 rounded-xl shadow-inner border border-gray-200">
                    <span className="text-[10px] font-bold text-gray-400 mr-2 uppercase tracking-tighter">US 환율</span>
                    <span className="text-sm font-black text-indigo-600">{formatMoney(exchangeRate)}원</span>
                  </div>
                </div>
                {assets.stocks.map(stock => {
                   const isUS = stock.market === '미국';
                   const buy = parseFloat(stock.buyPrice) || 0;
                   const cur = parseFloat(stock.currentPrice) || 0;
                   const qty = parseFloat(stock.quantity) || 0;
                   const rate = isUS ? exchangeRate : 1;
                   const profit = buy > 0 ? (((cur - buy) / buy) * 100).toFixed(2) : 0;
                   const currentVal = qty * cur * rate;
                   return (
                      <div key={stock.id} className="bg-white rounded-[2.5rem] p-7 shadow-sm border border-gray-100 relative mb-4">
                         <div className={`absolute top-0 right-0 ${parseFloat(profit) >= 0 ? 'bg-red-500' : 'bg-blue-500'} text-white px-5 py-1.5 rounded-bl-3xl font-black text-[10px]`}>{parseFloat(profit) >= 0 ? '+' : ''}{profit}%</div>
                         <div className="text-[10px] text-gray-400 font-bold mb-1 uppercase tracking-widest">{stock.market} / {stock.note}</div>
                         <div className="text-xl font-black text-gray-900 mb-4">{stock.name}</div>
                         <div className="flex justify-between items-end border-t border-gray-50 pt-4">
                            <div><div className="text-[9px] text-gray-400 font-bold mb-1 uppercase tracking-tighter">보유 {stock.quantity}개 / 평단 {isUS?'$':''}{stock.buyPrice}</div><div className="text-2xl font-black text-gray-900">{formatMoney(currentVal)}원</div></div>
                            <div className="text-right"><div className="text-[9px] text-gray-400 font-bold mb-1 uppercase tracking-tighter">현재가</div><div className="text-lg font-bold text-indigo-500">{isUS?'$':''}{stock.currentPrice}</div></div>
                         </div>
                      </div>
                   )
                })}
