    ...
    <div className={`min-h-screen font-sans text-gray-900 select-none pb-32 transition-colors duration-500 ${appBgColor}`}>
            ...
            <div className="bg-gradient-to-br from-blue-600 to-cyan-500 rounded-3xl p-4 text-white shadow-md relative overflow-hidden mt-1">
              ...
              <div className="flex justify-between items-end mb-2 relative z-10">
                ...
                <div className="text-right">
                  <div className="text-[8px] bg-white/20 px-1.5 py-0.5 rounded font-bold tracking-tight mb-1 inline-block">{selectedYear}년 누적: {formatCompactMoney(deliveryYearlyTotal)}</div>
          ...
          <div className="space-y-5 pb-28 pt-2 animate-in slide-in-from-right duration-500">
            ...
            <div className="bg-pink-50/80 rounded-3xl p-5 border border-pink-200/60 shadow-sm relative">
               <h3 className="text-xs font-black text-pink-500 mb-3 flex justify-between items-center">
                  ...
                  <button onClick={() => setIsMessageHistoryOpen(true)} className="text-gray-400 font-bold border-b border-gray-300 pb-0.5 active:text-pink-500">과거 내역</button>
               </h3>
               
               <div className="space-y-2 mb-3">
                  {activeMessages.length === 0 && <div className="text-center text-gray-400 font-bold text-[10px] py-4 bg-white/50 rounded-2xl border border-pink-100/50">새로운 메시지가 없습니다.</div>}
                     ...
                     <div key={m.id} className="bg-white p-3 rounded-2xl flex justify-between items-center shadow-sm border border-pink-100/50">
                        <div className="pr-3">
                           <div className="text-[9px] text-gray-400 font-bold mb-0.5 flex items-center gap-1">
                             {m.author === '시스템' ? <span className="bg-emerald-100 text-emerald-600 px-1.5 rounded">알림</span> : <span className="bg-pink-100 text-pink-600 px-1.5 rounded">{m.author}</span>}
                             {typeof m.createdAt === 'string' && m.createdAt.slice(5).replace('-','/')}
                           </div>
                           <div className="text-sm font-black text-gray-800 leading-tight break-all">{m.text}</div>
                        </div>
                        <button onClick={() => handleCheckMessage(m.id)} className="bg-gray-50 text-gray-500 border border-gray-200 px-3 py-2 rounded-xl text-[10px] font-black shrink-0 active:scale-90 transition-transform">확인</button>
                     </div>
                  ))}
               </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end justify-center z-[60] p-4 pb-8 overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-7 shadow-2xl animate-in slide-in-from-bottom duration-300 border border-pink-100">
            <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black text-gray-800">✨ 내역 기록</h2><button onClick={closeModals} className="bg-pink-50 text-pink-500 p-2.5 rounded-2xl border border-pink-100 shadow-sm"><X size={20}/></button></div>
