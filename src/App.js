...
function LedgerView({ ledger, setLedger, memos, setMemos, selectedYear, selectedMonth, currentMonthKey, todayStr, categories, setCategories, user, isManageMode }) {
  ...
  const deleteTransaction = async (id) => {
    if(!window.confirm('이 가계부 내역을 삭제하시겠습니까?')) return;
          ...
          <div className="bg-white rounded-[2rem] p-4 shadow-md border border-pink-200/60 animate-in slide-in-from-bottom-2">
             ...
             <div className="grid grid-cols-7 gap-1">
               {days.map((d, i) => {
                 ...
                 const dayData = dataByDate[`${selectedYear}-${String(selectedMonth).padStart(2,'0')}-${String(d).padStart(2,'0')}`] || { inc: 0, exp: 0 };
                 return (
                   <div key={`day-${i}`} className={`h-[55px] border rounded-xl p-0.5 flex flex-col items-center justify-start ${(dayData.inc>0||dayData.exp>0)?'border-pink-200 bg-pink-50/40 shadow-sm':'border-gray-100 bg-white'}`}>
      ...
      {ledgerSubTab === 'memo' && (
        <div className="space-y-3 animate-in slide-in-from-right duration-300">
           ...
           {memos.sort((a,b) => b.updatedAt.localeCompare(a.updatedAt)).map(memo => (
              <div key={memo.id} onClick={() => { setCurrentMemoId(memo.id); setMemoText(memo.text || ''); setIsMemoEditorOpen(true); }} className="bg-white rounded-3xl p-5 shadow-sm border border-gray-200/80 cursor-pointer relative">
            ...
            <div key={date} className="bg-white rounded-3xl p-4 shadow-sm border border-gray-200/80">
               ...
               <div className="space-y-2.5">
                {(groupedLedger[date]||[]).map(t => (
                  <div key={t.id} onClick={() => setExpandedLedgers(prev => ({...prev, [t.id]: !prev[t.id]}))} className="bg-gray-50/50 border border-gray-100/50 rounded-2xl cursor-pointer shadow-sm">
                    <div className="flex justify-between items-center p-3">
                      <div className="flex items-center gap-3 overflow-hidden flex-1">
                        ...
                        <div className="truncate pr-2">
                          <div className="text-[10px] font-bold text-gray-500 flex items-center gap-1">{t.category} {t.subNote && <ChevronDown size={10} className={`transition-transform duration-300 ${expandedLedgers[t.id] ? 'rotate-180' : ''}`} />}</div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 pl-2">
                        ...
                        {isManageMode && (
                          <div className="flex gap-1 ml-1">
                            <button onClick={(e) => { e.stopPropagation(); setFormData(t); setEditingLedgerId(t.id); setIsModalOpen(true); }} className="text-gray-400 hover:text-blue-500 bg-white p-2 rounded-xl border border-gray-100"><Edit3 size={14}/></button>
                            <button onClick={(e) => { e.stopPropagation(); deleteTransaction(t.id); }} className="text-gray-400 hover:text-pink-500 bg-white p-2 rounded-xl border border-gray-100"><Trash2 size={14}/></button>
                          </div>
                        )}
                      </div>
                    </div>
                    {expandedLedgers[t.id] && t.subNote && <div className="px-4 pb-4"><div className="bg-white p-3.5 rounded-xl border border-gray-100 text-[11px] font-bold text-gray-700 whitespace-pre-wrap">{t.subNote}</div></div>}
                  </div>
                ))}
               </div>
            </div>
  ...
  return (
    <div className="flex flex-col gap-2 pb-28 pt-1 animate-in fade-in duration-500">
      <div className="bg-white rounded-2xl p-2.5 shadow-sm border border-slate-200 flex justify-between items-center">
        ...
        <button onClick={() => { if(timerActive) {handleEndDelivery(); setIsDeliveryModalOpen(true);} else handleStartDelivery(); }} className={`px-5 py-2.5 rounded-xl font-black text-sm shadow-sm ${timerActive ? 'bg-slate-800 text-white' : 'bg-blue-600 text-white'}`}>
      ...
      <div className="grid grid-cols-2 gap-2 mt-1">
        {upcomingPaydays.length === 0 ? (
          ...
          upcomingPaydays.slice(0,2).map((pd, idx) => {
            ...
            return (
              <div key={pd} className={`bg-white rounded-2xl p-3.5 shadow-sm border ${idx === 0 ? 'border-blue-300 bg-blue-50/30' : 'border-slate-200'} flex flex-col justify-between`}>
          ...
          <div className="bg-white rounded-[2rem] p-5 shadow-sm border border-blue-200 animate-in slide-in-from-bottom-2 mt-1">
             ...
             <div className="grid grid-cols-7 gap-1">
               {days.map((d, i) => {
                 ...
                 const dayData = dataByDate[`${selectedYear}-${String(selectedMonth).padStart(2,'0')}-${String(d).padStart(2,'0')}`] || { amt: 0 };
                 return (
                   <div key={`day-${i}`} className={`h-[60px] border rounded-xl p-1 flex flex-col items-center justify-center ${dayData.amt>0?'border-blue-200 bg-blue-50/40 shadow-sm':'border-gray-100 bg-white'}`}>
                    ...
                    return (
                      <div key={d.id} className="flex justify-between items-center bg-slate-50/50 p-3 rounded-2xl hover:bg-blue-50/50 transition-colors border border-slate-100/50">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {isManageMode && <button onClick={() => { setDeliveryFormData({...d, amount: String(d.amount||''), count: String(d.count||''), startTime: d.startTime||'', endTime: d.endTime||''}); setEditingDeliveryId(d.id); setIsDeliveryModalOpen(true); }} className="text-gray-400 hover:text-blue-500 bg-white p-2 rounded-xl border"><Edit3 size={14}/></button>}
                          {isManageMode && <button onClick={() => deleteDailyDelivery(d.id)} className="text-gray-400 hover:text-red-500 bg-white p-2 rounded-xl border"><Trash2 size={14}/></button>}
                        </div>
                      </div>
                    );
                  })}
