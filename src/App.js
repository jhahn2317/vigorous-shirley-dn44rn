...
// --- 💡 앱 버전 및 업데이트 시간 ---
const APP_VERSION = "v7.6 (바텀시트 & 모바일 100% 최적화: 2026.05.01)";
...
function AppContent() {
  ...
  // 💡 기호 오류 완벽 해결 (숫자만 축약해서 반환. +-는 JSX에서 붙임)
  ...
  const filteredLedger = useMemo(() => {
    ...
    } else data = data.filter(t => t.date?.startsWith(`${selectedYear}-${monthStr}`));
  ...
  const handleDeliverySubmit = async (e) => {
    ...
    if (editingDeliveryId) {
      ...
      if (isFirebaseEnabled) {
        ...
        logEvent('배달 수정', `${newDel.date} ${newDel.earner} 수익 수정`);
      }
    } else {
      const adds = [];
    ...
    showToast("수익 기록 완료! 수고하셨습니다 👏"); closeModals();
  };
  ...
  const saveBatchDuties = async () => {
    ...
    if (isFirebaseEnabled) {
      ...
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'messages'), { author: '시스템', text: `현아가 ${dutyBatchMonth}월 근무표를 통째로 업데이트했어요! 🗓️`, createdAt: todayStr, isChecked: false });
      logEvent('근무표 일괄 등록', `${dutyBatchMonth}월 근무표를 업데이트했습니다.`);
    }
    setIsDutyBatchModalOpen(false);
  ...
  const handlePrepaySubmit = async (e) => {
    ...
    showToast("중도상환 내역이 기록되었습니다."); closeModals();
  };
  ...
  const handleAddCategory = async (type) => {
    ...
    if(newCat && newCat.trim() !== '') {
       if(isFirebaseEnabled && user) await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'categories'), {...categories, [type]: [...(categories[type]||[]), newCat.trim()]});
    }
  };
  
  const handleDeleteCategory = async (type, cat) => {
    if(window.confirm(`'${cat}' 카테고리를 정말 삭제하시겠습니까?`)) {
       if(isFirebaseEnabled && user) await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'categories'), {...categories, [type]: (categories[type]||[]).filter(c => c !== cat)});
    }
  };

  const updateSettings = async (field, value) => {
    if(isFirebaseEnabled && user) await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'preferences'), { ...userSettings, [field]: value });
  };
    ...
    <div className={`min-h-screen font-sans text-gray-900 select-none pb-32 transition-colors duration-500 ${appBgColor}`}>
      ...
      <header className="bg-white/90 backdrop-blur-md pt-12 pb-4 px-6 sticky top-0 z-30 flex justify-between items-center border-b border-gray-100 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setIsManageMode(!isManageMode)} className={`p-2.5 rounded-full transition-all ${isManageMode ? 'bg-indigo-600 text-white shadow-lg rotate-90' : 'bg-white text-gray-400 border border-gray-200 shadow-sm'}`}><Settings size={20}/></button>
        </div>
      </header>
        ...
        <div className="px-5 my-4 animate-in fade-in space-y-4">
           ...
           <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-md">
             ...
             <div className="space-y-3">
               <div className="flex justify-between items-center p-3 rounded-xl border bg-gray-50 border-gray-100">
                 <div>
                   <span className="text-[11px] font-bold text-gray-600 block mb-0.5">앱 시작 시 화면</span>
                   <span className="text-sm font-black text-indigo-700">{activeTab === 'ledger' ? '가계부' : activeTab === 'delivery' ? '배달수익' : activeTab === 'loans' ? '대출관리' : '우리가족'}</span>
                 </div>
                 <button onClick={() => { localStorage.setItem('hyunaDefaultTab', activeTab); showToast('이 기기의 초기화면이 설정되었습니다.'); }} className="bg-indigo-600 text-white text-[10px] px-3 py-2 rounded-lg font-bold active:scale-95 shadow-sm">
                   현재 탭으로 고정
                 </button>
               </div>
                    ...
                    return arr.map((d, i) => {
                      ...
                      return (
                        <div key={i} className={`h-[55px] border rounded-xl p-0.5 flex flex-col items-center justify-start ${(inc||exp)?'border-pink-200 bg-pink-50/40 shadow-sm':'border-gray-50 bg-white'}`}>
                          ...
                          {/* 💡 기호 중복 완벽 제거 (formatCompactMoney는 숫자만 주고 밖에서 기호 붙임) */}
                    ...
                    <div className="space-y-1.5">{(groupedDaily[date]||[]).map(d => {
                      ...
                      return (
                        <div key={d.id} className="flex justify-between items-center bg-gray-50/50 p-2.5 rounded-2xl text-xs hover:bg-blue-50 transition-colors">
                          <div className="flex items-center gap-2.5"><div className={`w-8 h-8 rounded-xl text-white flex items-center justify-center font-black text-[9px] ${d.platform === '배민' ? 'bg-[#2ac1bc]' : d.platform === '쿠팡' ? 'bg-[#111111]' : 'bg-gray-400'}`}>{d.platform}</div><div><div className="font-bold text-gray-800 text-xs">{d.earner} <span className="text-gray-400 text-[9px]">| {d.count}건 {d.startTime ? `(${d.startTime}~${d.endTime})` : ''}</span></div><div className={`text-[9px] font-bold mt-0.5 ${isPending ? 'text-blue-400' : 'text-gray-400'}`}>{isPending ? `${pDay.slice(5).replace('-','/')} 입금 대기` : '정산 완료'}</div></div></div>
            ...
            <form onSubmit={handleTransactionSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={formData.category} onChange={e=>setFormData({...formData, category:e.target.value})} className="w-full bg-gray-50 rounded-xl p-3 font-bold text-xs outline-none border border-gray-200">{(CATEGORIES[formData.type]||[]).map(c => <option key={c} value={c}>{c}</option>)}</select>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end justify-center z-[60]">
          <div className="bg-white w-full max-w-md rounded-t-[2rem] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 border-t border-emerald-100 pb-12">
            ...
            <form onSubmit={handleEventSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-[10px] font-black text-gray-400 ml-1 block uppercase mb-1">분류</label><select value={eventFormData.type} onChange={(e) => setEventFormData({...eventFormData, type: e.target.value})} className="w-full bg-gray-50 rounded-xl p-3 text-xs font-bold outline-none border border-gray-200 focus:ring-2 ring-emerald-200"><option value="가족일정">가족일정</option><option value="회식">회식</option><option value="기타">기타</option></select></div>
                <div><label className="text-[10px] font-black text-gray-400 ml-1 block uppercase mb-1">날짜</label><input type="date" value={eventFormData.date} onChange={e=>setEventFormData({...eventFormData, date:e.target.value})} className="w-full bg-gray-50 rounded-xl p-3 text-xs font-bold outline-none border border-gray-200 focus:ring-2 ring-emerald-200" /></div>
              </div>
              <div>
      ...
      {isDutyBatchModalOpen && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-end justify-center z-[70] p-4">
            <div className="bg-white w-full max-w-md rounded-t-[2rem] rounded-b-[2rem] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 max-h-[90vh] overflow-y-auto">
      ...
      {/* 💡 배달 기록 모달 (Bottom Sheet & 가로 겹침 방지 스택/그리드) */}
      {isDeliveryModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end justify-center z-[60]">
          <div className="bg-white w-full max-w-md rounded-t-[2rem] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300 border-t border-blue-500 max-h-[90vh] overflow-y-auto pb-12">
            ...
            <form onSubmit={async (e) => {
              ...
              <div className="grid grid-cols-3 gap-2 border-t border-gray-100 pt-3">
                 <div><label className="text-[9px] font-black text-gray-400 ml-1 block mb-1">시작</label><input type="time" value={deliveryFormData.startTime} onChange={e=>setDeliveryFormData({...deliveryFormData, startTime:e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 font-bold text-[10px] outline-none" /></div>
                 <div><label className="text-[9px] font-black text-gray-400 ml-1 block mb-1">종료</label><input type="time" value={deliveryFormData.endTime} onChange={e=>setDeliveryFormData({...deliveryFormData, endTime:e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 font-bold text-[10px] outline-none" /></div>
                 <div><label className="text-[9px] font-black text-gray-400 ml-1 block mb-1">날짜</label><input type="date" value={deliveryFormData.date} onChange={e=>setDeliveryFormData({...deliveryFormData, date:e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 font-bold text-[10px] outline-none" /></div>
              </div>
